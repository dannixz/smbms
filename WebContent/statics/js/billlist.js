	...
	$.ajax({
		...
		success:function(data){
			if(data.delResult == "true"){//删除成功：移除删除行
				...
				window.location.href=path+"/user/billShow";
...
$(function(){
	$(".viewBill").on("click",function(){
		...
		var obj = $(this);
		window.location.href=path+"/user/billView/"+ obj.attr("billid");
	});
	
	$(".modifyBill").on("click",function(){
		var obj = $(this);
		window.location.href=path+"/user/billmodify/"+ obj.attr("billid");
	});
	$('#no').click(function () {
		cancleBtn();
	});
